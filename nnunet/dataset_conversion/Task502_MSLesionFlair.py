...
if __name__ == '__main__':
    ...
    task_name = 'Task502_MSLesionFlair'
    ...
    for t in training_cases:
        unique_name =  os.path.basename(t).split('.')[0][4:]  # just the filename with the extension cropped away, so img-2.png becomes img-2 as unique_name
        input_segmentation_file = join(labels_dir_tr, t)
        input_image_file = join(images_dir_tr, t[4:])
    ...
    for ts in testing_cases:
        unique_name =  os.path.basename(ts).split('.')[0][4:]
        input_segmentation_file = join(labels_dir_ts, ts)
        input_image_file = join(images_dir_ts, ts[4:])
