...
if __name__ == '__main__':
    ...
    for t in training_cases:
        unique_name =  os.path.splitext(os.path.basename(t))[0]  # just the filename with the extension cropped away, so img-2.png becomes img-2 as unique_name
    ...
    print('Training Cases:', len(testing_cases))

    for ts in testing_cases:
        unique_name =  os.path.splitext(os.path.basename(ts))[0]
    ...
    generate_dataset_json(join(target_base, 'dataset.json'), target_imagesTr, target_imagesTs, ('CXR',),
                          labels={0: 'background', 1: 'left lung',2: 'right lung',3: 'heart', 4: 'right clavicle',5: 'left clavicle' }, dataset_name=task_name, license='hands off!')
